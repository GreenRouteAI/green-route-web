...
import { RefineListView, CategoryStatus, CustomTooltip } from '../../components';
...
export const CategoryList = () => {
  ...
  const { data: productsData, isLoading: productsIsLoading } = useList<IProduct, HttpError>({
    resource: 'products',
    pagination: {
      mode: 'off',
    },
    ...
    () => [
      ...
      },
      {
        ...
        renderCell: function render({ row }) {
          const categoryProducts = products.filter(product => product.category.id === row.id);
          return (
  ...
  return (
    <RefineListView>
      <Paper>
        <DataGrid {...dataGridProps} sx={{}} columns={columns} autoHeight hideFooter />
      </Paper>
    </RefineListView>
  );
};
