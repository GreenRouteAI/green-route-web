  ...
  const columns = useMemo<GridColDef<ICourier>[]>(
    () => [
      ...
      },
      {
        ...
        width: 64,
        renderCell: function render({ row }) {
          return <Avatar alt={`${row.name} ${row.surname}`} src={row.avatar?.[0]?.url} sx={{ width: 32, height: 32 }} />;
        },
      },
      {
        field: 'name',
  ...
  return (
    <>
      <RefineListView
        ...
        <Paper>
          <DataGrid {...dataGridProps} columns={columns} sx={{}} autoHeight pageSizeOptions={[10, 20, 50, 100]} />
        </Paper>
      </RefineListView>
      {children}
    </>
