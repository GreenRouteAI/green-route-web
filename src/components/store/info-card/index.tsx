...
export const StoreInfoCard = (props: Props) => {
  ...
  return (
    <Paper>
      ...
      />
      <Divider />
      <Info icon={<AccountCircleOutlinedIcon />} label={t('stores.fields.email')} value={email} />
      <Divider />
      <Info icon={<PhoneOutlinedIcon />} label={t('stores.fields.gsm')} value={gsm} />
    </Paper>
  );
};
...
const Info = ({ icon, label, value, sx }: InfoProps) => {
  ...
  return (
    <Box display='flex' alignItems='flex-start' justifyContent='flex-start' p='16px 0px 16px 24px' sx={sx}>
      <Box
        mr='8px'
        display='flex'
      ...
      {value ?? <Skeleton variant='text' sx={{ fontSize: '1rem', width: '200px' }} />}
    </Box>
  );
};
