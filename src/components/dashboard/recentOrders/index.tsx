...
import { UpdatePasswordFormTypes, useNavigation, useTranslate, useUpdate, useUpdatePassword } from '@refinedev/core';
...
export const RecentOrders: React.FC = () => {
  ...
  const { mutate: updatePassword } = useUpdatePassword<Record<string, string>>();
            ...
            <Stack spacing={1}>
              {products.map(product => (
                <Typography key={product.id} whiteSpace='nowrap'>
                  {product.name}
                  <Typography component='span' color='GrayText' sx={{ ml: '4px' }}>
                    x{product.count}
                  </Typography>
                </Typography>
              ))}
