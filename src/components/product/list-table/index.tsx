...
export const ProductListTable = (props: Props) => {
  ...
  const columns = useMemo<GridColDef<IProduct>[]>(
    () => [
      ...
      },
      {
        ...
        filterable: false,
        renderCell: function render({ row }) {
          const category = props.categories.find(category => category.id === row.category.id);
  ...
  return <DataGrid {...props.dataGridProps} sx={{}} columns={columns} autoHeight pageSizeOptions={[12, 24, 48, 96]} />;
};
